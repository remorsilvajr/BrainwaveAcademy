        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center">
          ...
          <div className="mt-6 space-y-2 border-t border-gray-100 pt-4 text-left text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-gray-900">{application.application_ref}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">Submitted</span>
              <span className="font-semibold text-gray-900">{formatDateLong(application.submitted_at)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 md:col-span-2">
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="rounded-lg bg-gray-50 p-3">
              <p className="text-xs text-gray-400">Date of Birth (Age)</p>
            ...
            <div className="rounded-lg bg-gray-50 p-3">
              ...
              <p className="text-sm font-medium capitalize text-gray-900">{application.student_gender}</p>
            </div>
          </div>

          <h2 className="mt-6 border-b border-gray-100 pb-3 text-lg font-bold text-gray-900">
            Guardian Info
          </h2>
          <div className="mt-4 rounded-lg border border-gray-200 p-4">
            <p className="font-medium text-gray-900">
              ...
              {parentProfile?.relationship_to_student && (
                <span className="text-xs text-gray-500">({parentProfile.relationship_to_student})</span>
              )}
            </p>
            {parentProfile?.email && <p className="mt-1 text-sm text-gray-600">{parentProfile.email}</p>}
            {parentProfile?.phone_number && <p className="text-sm text-gray-600">{parentProfile.phone_number}</p>}
          </div>
            ...
            {documentOrder.map((type) => {
              ...
              return (
                <div key={type} className="flex items-center justify-between rounded-lg border border-gray-200 px-3 py-2 text-sm">
                  <span className="text-gray-700">{documentShortLabels[type]}</span>
                  <span
