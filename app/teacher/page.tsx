  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">Here is an overview of your classroom for today.</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div className="rounded-2xl border border-gray-200 bg-white p-6">
          ...
          <p className="mt-3 text-3xl font-bold text-gray-900">
            {presentCount} <span className="text-base font-normal text-gray-500">/ {roster.length} Present</span>
          </p>
          ...
          <Link
            ...
            className="mt-4 inline-block rounded-lg border border-[#0b1b62] px-4 py-2 text-sm font-semibold text-[#0b1b62] hover:bg-[#0b1b62]/5"
          >
            Take Attendance →
          </Link>
        </div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6">
          <div className="flex items-center justify-between">
            ...
            {pendingStudents.length > 0 && (
              <span className="rounded-full bg-amber-50 px-3 py-1 text-sm font-medium text-amber-700">Due Soon</span>
            )}
          </div>
          <p className="mt-3 text-3xl font-bold text-gray-900">
            {pendingStudents.length} <span className="text-base font-normal text-gray-500">Students</span>
          </p>
          <p className="mt-1 text-sm text-gray-500">Have incomplete 6-domain evaluations</p>
          <a
            href="#assessments"
      ...
      <div id="assessments" className="rounded-2xl border border-gray-200 bg-white p-6">
        <h2 className="text-lg font-bold text-gray-900">Pending Student Assessments (6 Domains of Learning)</h2>
        <div className="mt-4 divide-y divide-gray-100">
            ...
            return (
              <div key={s.id} className="flex items-center justify-between gap-4 py-3">
                <div>
                  ...
                  <p className="text-xs text-gray-500">{domains}/{milestoneCategoryOrder.length} Domains Complete</p>
                </div>
                <Link
          ...
          {roster.length === 0 && <p className="py-6 text-center text-sm text-gray-500">No students on file yet.</p>}
        </div>
      </div>
    </div>
  )
